...
// Define a type for our expected API error shape
interface ApiError {
  error: string;
}
...
export default function DashboardPage() {
  ...
  const handleQuery = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (axios.isAxiosError(err)) {
        const axiosError = err as AxiosError<ApiError>;
        alert(axiosError.response?.data?.error || 'Query failed.');
      }
    } finally {
      setIsQuerying(false);
    }
  ...
  const handleVisualize = async () => {
    ...
    try {
      ...
      if (axios.isAxiosError(err)) {
        const axiosError = err as AxiosError<ApiError>;
        alert(axiosError.response?.data?.error || 'Visualization failed.');
      }
    } finally {
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
          ...
          {!datasetId ? (
            <div className="bg-black/30 p-8 rounded-lg backdrop-blur-lg border border-white/10 shadow-lg" style={{ boxShadow: '0 4px 24px 0 rgba(0,0,255,0.5)' }}>
              ...
              <p className="text-gray-400 mb-6">Upload a CSV or Excel file to begin your analysis.</p>
              <form onSubmit={handleUpload}>
                <div className="flex flex-col sm:flex-row items-center space-y-4 sm:space-y-0 sm:space-x-4">
                  <input type="file" onChange={handleFileChange} className="block w-full text-sm text-gray-300 file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-semibold file:bg-white/10 file:text-white hover:file:bg-white/20" accept=".csv,.xlsx" />
